...
@Component(Kiosk2ClothDistributeHandler.BEANNAME)
public class Kiosk2ClothDistributeHandler extends CaptureButtonBaseHandler implements ECSpecHandler
{
	private static final Logger log4j = Logger.getLogger(Kiosk2ClothDistributeHandler.class);
	
	public static final String BEANNAME = "Kiosk2ClothDistributeHandler";
	public static final String SPECNAME = "Kiosk2ClothDistributeHandler";
	...
	public void handle(List<String> rawHexs, ECReports ecReports)
	{
		...
		if (enable)
		{
			...
			// 2. if have, get newly-captured-ready-status-RFID info from DB
			if (rfidListNew.size() > 0)
			{
				List<Cloth> clothList = this.getClothListByRfid(rfidListNew);
				
				// 3. put all newly-captured-ready-status-RFID to map
				Cloth curCloth = null;
				String curRfid = null;
				synchronized (this)
				{
					for (int i = 0; i < clothList.size(); i++)
					{
						...
						// Only clothes at "Ready" status are added
