...
@Results({
	@Result(name="getReportPage", location="monthly.laundry.recv.report", type="tiles"), 
	@Result(name="input", location="monthly.laundry.recv.report", type="tiles"),
	
	@Result(name="makeReport",  type="jasper", params={
			"location", 	"/jasper_report/reportMonthlyLaundryRecv.jasper", 
			"dataSource", 	"reportList",
			"format", 		"${format}",
			"documentName",	"Monthly_Laundry_Recv_Report"
	}),
})
@InterceptorRefs({
...
public class MonthlyLaundryRecvReportAction extends BaseActionReport
{
	private static final long serialVersionUID = 5854852324575959370L;
	private static final Logger log4j = Logger.getLogger(MonthlyLaundryRecvReportAction.class);
	
	private List<ReportObject1> reportList;
	// private String yearMonthStr;		// already defined in superclass
	
	public MonthlyLaundryRecvReportAction()
	{
	...
	public String exportImpl() throws Exception
	{
		...
		List<Receipt> receiptList = findReceipt(this.dateFrom, this.dateTo, ReceiptType.Collect);
	...
	private String exportExcel()
	{
		try
		{
			...
			this.response.setHeader("Content-Disposition", "attachment; filename=Monthly_Laundry_Recv_Report.xls");
			...
			Label label = new Label(0, 0, "\u6bcf\u6708\u6c61\u8863\u7269\u6536\u53d6\u5831\u544a", titleCellFormat);
			sheet.addCell(label);
